...
def annotate_custom_transcripts(standard_transcript_dict, gene_junction_dict):

	for transcript in standard_transcript_dict:

		standard_transcript_dict[transcript]["gene_ambiguous_in_annotation"] = False

		transcript_junctions = splice_lib.get_junctions(standard_transcript_dict[transcript]["exons"])

		junction_keys = [standard_transcript_dict[transcript]["chrom"] + "_" + "_".join(map(str, junction)) + "_" + standard_transcript_dict[transcript]["strand"] for junction in transcript_junctions]
		...
		for junction_key in junction_keys:
			...
			if junction_key in gene_junction_dict:
				...
				if len(gene_junction_dict[junction_key]) > 1:
					...
					standard_transcript_dict[transcript]["gene_ambiguous_in_annotation"] = True
		...
		if len(compatible_genes) > 0:
			...
			for junction_key in junction_keys:
				...
				if junction_key in gene_junction_dict:
					...
					gene_junction_dict[junction_key] = list(set(compatible_genes + list(gene_junction_dict[junction_key])))

				else:

					gene_junction_dict[junction_key] = compatible_genes

			standard_transcript_dict[transcript]["gene"] = ",".join(compatible_genes)




def find_matches(event_dict, event_keys, gene_junction_dict):
	...
	for event in event_keys:
		...
		for junction in [list(i) for i in set(tuple(i) for i in (event_dict[event]["included_junctions"] + event_dict[event]["excluded_junctions"]))]:

			junction_key = event_dict[event]["chrom"] + "_" + "_".join(map(str,junction)) + "_" + event_dict[event]["strand"]
...
def main(args, event_dict = None):
	...
	parser.add_argument('--event_gtf', type = str, help = "GTF file of species 1 reference events")
	parser.add_argument('--gene_gtf', type = str, help = "GTF file of full genes - GENCODE format expected", required = True)
	parser.add_argument('--transcript_gtf', type = str, help = "Optional GTF file of transcripts e.g. generated by StringTie - allows more comprehensive event assignment as many-junction transcripts are more likely to have overlap with annotated genes")
	parser.add_argument('--prefix', type = str, help = "Prefix to render output file unique", required = True)
	parser.add_argument('--outdir', type = str, help = "Path to output directory")
	parser.add_argument('--chrom_transcript', type = str, help = "File associating chromosome names and transcript ids - useful for changing chrom IDs (e.g. from ensembl to ucsc")
	parser.add_argument('--transcript_gene', type = str, help = "File associating genes to transcript ids - useful when gene IDs (symbols) not contained in original gtf")
	parser.add_argument('--suppress_output', action = "store_true", help = "Suppress output files")
	...
	if transcript_gtf != None:
		...
		annotate_custom_transcripts(standard_transcript_dict, gene_junction_dict)
	...
	if event_gtf is not None or event_dict is not None:
		...
		if transcript_gtf != None:
			...
			new_matched_results = find_matches(events_dict, unmatched, gene_junction_dict)
